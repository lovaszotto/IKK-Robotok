...
def read_docx(file_path):
    try:
        ...
        if paragraph_count == 0 and table_count == 0:
            msg = f"[HIBA] Üres vagy nem szabványos szöveg! Paragrafusok száma 0 és táblázatok száma is 0: {file_path}"
            log_console(msg)
            return msg
        full_text = []
        seen = set()
            ...
            for row in table.rows:
                for cell in row.cells:
                    if id(cell) not in seen:
                        ...
                        if text:
                            # Eltávolítjuk az aposztrófokat és idézőjeleket
        ...
        #    log_console(msg)
        #log_console(">>>>>>>>>>> DOCX végigolvasva<<<<<<<<<<<<<<<")
        return result
    except Exception as e:
        #import traceback
        #tb = traceback.format_exc()
        log_console(f"[HIBA] DOCX beolvasás sikertelen: {file_path} ({e})")
        return "[HIBA] DOCX beolvasás sikertelen!"+str(e)
