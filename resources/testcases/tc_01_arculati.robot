...
Test Case 01 - Arculati Elemek Ellenorzese
    ...
    TRY
        ...
        IF    '${first_paragraph}'!=''   
            ${is_success}=    Set Variable    ${is_success}    ${False}
    ...
    TRY
        ...
        IF    '${fourth_paragraph}'!='Témák kézirata'   
            ${is_success}=    Set Variable    ${False}
    ...
    #ha ${is_success} false, akkor írja be a megjegyzésbe az err_msg-t (F3 cella)
    IF    ${is_success}==${False}
        Log To Console    [01] Megjegyzés írása F3-ba: ${err_msg}
        Fill Excel Cell    ${excel_file}    ${sheet_name}    3    6    ${err_msg}
    END
