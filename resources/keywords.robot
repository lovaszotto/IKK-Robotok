...
Create_K_ell_Excel
    ...
    IF    ${file_exists}
        ${doc_zip_ok}=    Run Keyword And Return Status    Evaluate    __import__('zipfile').ZipFile(r'''${activeExcelFile}''').testzip() is None    modules=zipfile
        ${web_exists}=    Run Keyword And Return Status    File Should Exist    ${web_activeExcelFile}
        ${web_zip_ok}=    Set Variable    ${False}
        IF    ${web_exists}
            ${web_zip_ok}=    Run Keyword And Return Status    Evaluate    __import__('zipfile').ZipFile(r'''${web_activeExcelFile}''').testzip() is None    modules=zipfile
        END
        IF    ${doc_zip_ok} and ${web_exists} and ${web_zip_ok}
        ...
        END
        Log String To Console    \n\[WARNING] Meglévő Excel fájl sérült vagy hiányos, újragenerálás indul.
        Run Keyword If    ${file_exists}    Remove File    ${activeExcelFile}
        Run Keyword If    ${web_exists}    Remove File    ${web_activeExcelFile}
    END
    ...
    IF    ${template_exists}
        ...
        TRY
            ...
            IF    ${media_files_count} > 0
                ...
                ${copy_script}=    Set Variable    $ErrorActionPreference='Stop'; function Normalize([string]$s){ return (($s.ToLower()) -replace '[^a-z0-9]','') }; $srcPath='${SRC_FILE}'; $dstPath='${DST_FILE}'; $requested='${SRC_SHEET}'; $dstName='${DST_SHEET}'; $excel=$null; $srcWb=$null; $dstWb=$null; $ok=$false; try { $excel=New-Object -ComObject Excel.Application; $excel.Visible=$false; $excel.DisplayAlerts=$false; $srcWb=$excel.Workbooks.Open($srcPath); $dstWb=$excel.Workbooks.Open($dstPath); $candidates=@($requested, "$requested-kapott", ($requested -replace '_MK','-MK'), ($requested -replace '_MK','-MK-kapott')); $srcWs=$null; foreach($n in $candidates){ try { $srcWs=$srcWb.Worksheets.Item($n); if($srcWs){ break } } catch {} }; if(-not $srcWs){ $reqNorm=Normalize $requested; foreach($ws in $srcWb.Worksheets){ $nm=Normalize $ws.Name; if($nm -eq $reqNorm -or $nm.StartsWith($reqNorm) -or $nm.Contains($reqNorm)){ $srcWs=$ws; break } } }; if(-not $srcWs){ $names=@(); foreach($ws in $srcWb.Worksheets){ $names += $ws.Name }; throw "Worksheet '$requested' does not exist. Available: $($names -join ', ')" }; try { $old=$dstWb.Worksheets.Item($dstName); $old.Delete() } catch {}; $srcWs.Copy([System.Type]::Missing, $dstWb.Worksheets.Item($dstWb.Worksheets.Count)); $newWs=$dstWb.Worksheets.Item($dstWb.Worksheets.Count); $newWs.Name=$dstName; $dstWb.Save(); $ok=$true; Write-Output "SUCCESS: $($srcWs.Name) -> $dstName" } finally { if($srcWb){ $srcWb.Close($false) }; if($dstWb){ $dstWb.Close($ok) }; if($excel){ $excel.Quit(); [System.Runtime.InteropServices.Marshal]::ReleaseComObject($excel) | Out-Null; [GC]::Collect(); [GC]::WaitForPendingFinalizers() } }
                ...
                ELSE
                    ...
                    ${mark_script}=    Set Variable    $ErrorActionPreference='Stop'; $path='${DST_FILE}'; $sheet='${DST_SHEET}'; $excel=$null; $wb=$null; $ok=$false; try { $excel=New-Object -ComObject Excel.Application; $excel.Visible=$false; $excel.DisplayAlerts=$false; $wb=$excel.Workbooks.Open($path); $ws=$wb.Worksheets.Item($sheet); $row=5; while($true){ $cellA=$ws.Cells.Item($row,1).Value2; if(-not $cellA){ break }; if($cellA -ne ''){ $ws.Cells.Item($row,9).Value2='x' }; $row++ }; $wb.Save(); $ok=$true; Write-Output "SUCCESS: Marked rows in '$sheet'" } finally { if($wb){ $wb.Close($ok) }; if($excel){ $excel.Quit(); [System.Runtime.InteropServices.Marshal]::ReleaseComObject($excel) | Out-Null; [GC]::Collect(); [GC]::WaitForPendingFinalizers() } }
                   ...
                   #írd ki a konzolra a PROMISED_TARTALOM és PROMISED_INDEX értékét, hogy ellenőrizhető legyen a helyességük
                    ...
                    ${get_promised_script}=    Set Variable    $ErrorActionPreference='Stop'; $path='${DST_FILE}'; $sheet='${DST_SHEET}'; $excel=$null; $wb=$null; try { $excel=New-Object -ComObject Excel.Application; $excel.Visible=$false; $excel.DisplayAlerts=$false; $wb=$excel.Workbooks.Open($path); $ws=$wb.Worksheets.Item($sheet); $row=5; $result=@(); $idx=@(); while($true){ $cellA=$ws.Cells.Item($row,1).Value2; if(-not $cellA){ break }; if($cellA -ne '' -and $cellA.ToString().ToLower().Contains('.mp4')){ $cellD=$ws.Cells.Item($row,4).Value2; if($cellD){ $result += $cellD.ToString().ToLower(); $idx += $row } }; $row++ }; @($result,$idx) | ConvertTo-Json -Compress -Depth 3 } finally { if($wb){ $wb.Close($false) }; if($excel){ $excel.Quit(); [System.Runtime.InteropServices.Marshal]::ReleaseComObject($excel) | Out-Null; [GC]::Collect(); [GC]::WaitForPendingFinalizers() } }
            ...
            ELSE
                ...
                ${create_script}=    Set Variable    $ErrorActionPreference='Stop'; $path='${dstPath}'; $sheet='${dstName}'; $excel=$null; $wb=$null; $ok=$false; try { $excel=New-Object -ComObject Excel.Application; $excel.Visible=$false; $excel.DisplayAlerts=$false; $wb=$excel.Workbooks.Open($path); $ws=$wb.Worksheets.Add(); $ws.Name=$sheet; $ws.Cells.Item(1,1).Value2 = "Nincs média ellenőrző fájl"; $wb.Save(); $ok=$true; Write-Output "SUCCESS: created '$sheet' with message" } finally { if($wb){ $wb.Close($ok) }; if($excel){ $excel.Quit(); [System.Runtime.InteropServices.Marshal]::ReleaseComObject($excel) | Out-Null; [GC]::Collect(); [GC]::WaitForPendingFinalizers() } }
...
Hide Excel Sheet
    ...
    [Arguments]    ${excel_file}    ${sheet_name}
    ${ps_code}=    Set Variable    $ErrorActionPreference='Stop'; $path='${excel_file}'; $sheet='${sheet_name}'; $excel=$null; $wb=$null; $ok=$false; try { $excel=New-Object -ComObject Excel.Application; $excel.Visible=$false; $excel.DisplayAlerts=$false; $wb=$excel.Workbooks.Open($path); $ws=$wb.Worksheets.Item($sheet); $ws.Visible=0; $wb.Save(); $ok=$true; Write-Output "SUCCESS: hidden '$sheet'" } finally { if($wb){ $wb.Close($ok) }; if($excel){ $excel.Quit(); [System.Runtime.InteropServices.Marshal]::ReleaseComObject($excel) | Out-Null; [GC]::Collect(); [GC]::WaitForPendingFinalizers() } }
...
Copy Excel Sheet
    ...
    ${ps_code}=    Set Variable    $ErrorActionPreference='Stop'; $path='${excel_file_ps}'; $src='${source_sheet_ps}'; $dst='${target_sheet_ps}'; $excel=$null; $wb=$null; $srcWs=$null; $newWs=$null; $ok=$false; try { $excel=New-Object -ComObject Excel.Application; $excel.Visible=$false; $excel.DisplayAlerts=$false; $excel.AlertBeforeOverwriting=$false; $wb=$excel.Workbooks.Open($path,0,$false,[Type]::Missing,[Type]::Missing,[Type]::Missing,$true,[Type]::Missing,[Type]::Missing,$false,[Type]::Missing,$false,$false); $srcWs=$wb.Worksheets.Item($src); try { $old=$wb.Worksheets.Item($dst); $old.Delete() } catch {}; $srcWs.Copy([Type]::Missing,$wb.Worksheets.Item($wb.Worksheets.Count)); $newWs=$wb.Worksheets.Item($wb.Worksheets.Count); $newWs.Name=$dst; $wb.Save(); $ok=$true; Write-Output 'SUCCESS' } finally { if($wb){ $wb.Close($ok) }; if($newWs){ [System.Runtime.InteropServices.Marshal]::ReleaseComObject($newWs) | Out-Null }; if($srcWs){ [System.Runtime.InteropServices.Marshal]::ReleaseComObject($srcWs) | Out-Null }; if($wb){ [System.Runtime.InteropServices.Marshal]::ReleaseComObject($wb) | Out-Null }; if($excel){ $excel.Quit(); [System.Runtime.InteropServices.Marshal]::ReleaseComObject($excel) | Out-Null; [GC]::Collect(); [GC]::WaitForPendingFinalizers() } }
    ${result}=    Run Process    powershell    -NoProfile    -ExecutionPolicy    Bypass    -Command    ${ps_code}
